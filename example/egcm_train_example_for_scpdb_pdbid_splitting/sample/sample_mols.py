...
for id,complexstruc in tqdm(enumerate(pocketlist)):
    try:
        #ligpath='/mnt/home/myxu/Database/'+complexstruc['name'].strip('/home/myxu/Workspace/Database/')+'/_ligand.sdf'
	#ligmol=Chem.SDMolSupplier(ligpath)[0]
        if complexstruc['rdkitsmiles'] and len(complexstruc[descriptor])>0:
            #mollist.append(ligmol)
            #smiles=Chem.MolToSmiles(ligmol)
